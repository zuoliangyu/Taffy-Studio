...
# browser (frontend embedded via rust-embed). Output: dist-web/taffy-web
...
mkdir -p dist-web
cp target/release/taffy-web dist-web/
echo '==> Done: dist-web/taffy-web'
echo '    Run it:  ./dist-web/taffy-web   (add --host 0.0.0.0 to expose on LAN)'

if [ "${RUN:-0}" = "1" ]; then
  exec ./dist-web/taffy-web
fi
